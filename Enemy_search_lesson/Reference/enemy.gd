...
#get search area,Needs to be attached to a NODE, else it will follow enemy as it moves
@onready var searchable_area = get_node("Path_anchor/Path3D/PathFollow3D")
...
#I increased the enemies ear collision shape 2X. Noise_emitter no longer needed
var sound_loc 
...
func check_alert_state(delta):
	...
	if in_pursuit:
		...
		search_for_player_last_pos(delta)
		#pursuit_state(delta)
		
#added
func search_for_player_last_pos(delta):
	
	await get_tree().process_frame
	
	var dir_dir 
	#added get the searhable area
	
	nav_agent.target_position =  searchable_area.global_position
	...
	#stops error
	if global_transform.origin.is_equal_approx(global_transform.origin + dir_dir):
...
#added player_near_window as parameter
func listen_for_player(player_noise):
	...
	if not result.is_empty():
		...
		#added player_near_window as conditional. if true. no sound reduction
		if result["collider"].is_in_group("player") or player_near_window:
			
			#checks the players sound threshold,if over pursue player
			
			if player_noise >= 5:
				$end_search.start()
				#added sound_loc, gets players pos and sets searchable area to that pos
				searchable_area.global_position = Global.player_current_pos
				in_pursuit = true
				
	#Some object collision is blocking the player from the enemy- IE. a wall
		
		#if not near_window, or window is closed. reduce sound
		if not result["collider"].is_in_group("player") or not player_near_window:
...
func check_doors_and_windows():
	...
	for dw in find_openings:
		...
		if dw.is_in_group("sound_tunnel_area"):
			...
			#added gets the return var from sound_tunnel
			
			if result["collider"].is_in_group("sound_tunnel_area"):
				#Needs to double check group for some reason??? 
				#crashes if collides with another staticbody3d
			
				if result["collider"].check_door_window_open() and result["collider"].check_player_sound():
...
func _on_search_timer_timeout() -> void:
	...
	#added, triggers a new pos to search for 
	#set to 3 seconds,autostart
	
	searchable_area.progress_ratio = randf()


func _on_end_search_timeout() -> void:
